...
def guessing_game(mode):
    if mode == "User Guessing":
        ...
        # Initialize the session state for attempts and success status
        ...
        # Input for user to think of a number
        ...
        # Computer guesses a number within the range
        if sl.session_state.attempts_left > 0 and user_number:
            ...
            if sl.session_state.computer_guess == user_number:
                ...
                sl.session_state.attempts_left = 0  # End game
            else:
...
if sl.button("Start Game"):
    sl.session_state.clear()  # Reset state on game start
    guessing_game(mode)